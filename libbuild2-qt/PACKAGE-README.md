...
This module provides compilation support for the Qt compilers `moc`, `rcc`,
and `uic`.
...
A compiler submodule can be loaded directly:
...
Otherwise, if all three compilers are going to be used, the `qt` submodule
...
Note that the Qt version must be specified before any of the submodules are
loaded. This is done by setting the `qt.version` variable to the Qt major
version number. Valid values are 5 and 6 (but note that the `build2` Qt5
packages are no longer maintained).

This module does not depend on any Qt packages so user projects need to add
the required Qt dependencies to their package `manifest`. For example:
...
# Dependencies on the primary source files and libraries.
#
exe{hello}: {hxx cxx}{hello} $libs

# Dependencies on the outputs of the Qt compilers.
#
exe{hello}: hxx{ui_hello} moc{hello} cxx{qrc_hello}

# Dependencies of the Qt compiler outputs on their input
# files.
#
...
This module runs `moc` (the Qt Meta-object Compiler) on C++ header and source
files.
...
Options that will be passed directly to `moc`.

#### `qt.moc.auto_preprocessor`

Default value: `true`.

Fallback variable used if any of the other `qt.moc.auto_*` variables are null
or undefined.

#### `qt.moc.auto_poptions`

If `true`, automatically pass the project poptions (`cc.poptions` and
`cxx.poptions`) to `moc`.

#### `qt.moc.auto_predefs`

If `true`, automatically pass the C++ compiler's pre-defined macros to `moc`.
...
The `moc{}` target type represents a C++ source file generated by `moc` from a
`cxx{}` input. It has the default extension `.moc` which can be customized
with the `extension` target type-specific variable.

#### `automoc{}`

The `hxx{}` and `cxx{}` prerequisites of `automoc{}` targets are scanned for
Qt meta-object macros and `moc` is then run on those prerequisites that
matched. See below for more information.

### Using `moc` with the `automoc{}` Target Type
```
exe{hello}: {hxx cxx}{hello} automoc{hello}
automoc{hello}: {hxx cxx}{**}
```
